...
let HotNewRecommendSQL = poolModule.HotNewRecommendSQL;
...
exports.getBooks = (req,res) => {
    ...
    // 获取距离现在
    pool.query(HotNewRecommendSQL.selectTodayBooks,[dayStartTime.getTime(),dayEndTime.getTime()], (err, rows)=>{
        if (err){
            res.send(err);
            throw err;
        }
        ...
        if(rows.length > 0){
            ...
            let book_ids = "";
            let books =[];
            pool.query(BookReadingDataSQL.selectHotNewBooks,[Date.now()-1000*60*60*30,Number(req.params.num)],(err,rows)=>{
                ...
                if(rows.length>0){
                    ...
                    for(let i=0;i<booksLength;i++){
                        ...
                        pool.query(BookSQL.selectOneBook,[id],(err,rows)=>{
                            ...
                            console.log(i,id,booksLength,book_ids);
                            books.push(rows[0]);
                            index++;
                            if(index === booksLength){
                                // 将获取的book_ids插入到heavy_recommend中去
                                pool.query(HotNewRecommendSQL.insert,[Date.now(),Date.now(),book_ids],(err,rows)=>{
                                    if(err) throw err;
                                    res.send(books);
                                    res.end();
                                });
