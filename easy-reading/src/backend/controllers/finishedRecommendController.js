...
let FinishedRecommendSQL = poolModule.FinishedRecommendSQL;
...
exports.getBooks = (req,res) => {
    ...
    // 获取距离现在
    pool.query(FinishedRecommendSQL.selectTodayBooks,[dayStartTime.getTime(),dayEndTime.getTime()], (err, rows)=>{
        if (err){
            res.send(err);
            throw err;
        }
        ...
        if(rows.length > 0){
            ...
            let book_ids = "";
            let books =[];
            pool.query(BookReadingDataSQL.selectFinishedBooks,[Number(req.params.num)],(err,rows)=>{
                    ...
                    for(let i=0;i<booksLength;i++){
                        ...
                        pool.query(BookSQL.selectOneBook,[id],(err,rows)=>{
                            ...
                            if(index === booksLength){
                                ...
                                pool.query(FinishedRecommendSQL.insert,[Date.now(),Date.now(),book_ids],(err,rows)=>{
                                    if(err) throw err;
                                    res.send(books);
                                    res.end();
                                });
